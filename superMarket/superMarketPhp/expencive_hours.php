        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="page-header clearfix">
                        <h2 class="pull-left">Highest Money Spending Hours</h2>
                    </div>
                    <?php
                    // Include config file
                    ...
                    $sql = "SELECT DISTINCT h, sum(totalCost) over (partition by h) as totalspend  from 
                    (SELECT hour(date_time) as h, totalCost from purchase) as a 
                        ...
                        if(mysqli_num_rows($result) > 0){
                            echo "<table class='table table-bordered table-striped'>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                        echo "<tr>";
                                        echo "<td>" . $row['h'] ." - " , intval($row['h']) + 1, "</td>";
                                        echo "<td>" . number_format((float)$row['totalspend'], 2, '.', '') ." €". "</td>";
                                        $temp = $temp + 1;
                                    echo "</tr>";
                                }
                                echo "</tbody>";                            
