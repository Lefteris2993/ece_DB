        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="page-header clearfix">
                        <h2 class="pull-left">Turnover by sex (single purchase)</h2>
                    </div>
                    <?php
                    // Include config file
                    ...
                    $sql = "SELECT gender, totalturn / COUNT(*) as mean FROM
                    (SELECT purchase.totalCost, customer.gender, sum(totalCost) over (partition by gender) as totalturn FROM purchase, customer
                    WHERE customer.cardID = purchase.cardID and (customer.gender = 'male' or customer.gender = 'female')) as a
                    GROUP BY gender";
                    if($result = mysqli_query($link, $sql)){
                        if(mysqli_num_rows($result) > 0){
                            echo "<table class='table table-bordered table-striped'>";
                                ...
                                echo "<tr>";
                                    echo "<th>Gender</th>";
                                    echo "<th>Turnover (mean value)</th>";
                                echo "</tr>";
                                echo "</thead>";
                                echo "<tbody>";
                                while($row = mysqli_fetch_array($result)){
                                        ...
                                        echo "<td>" . number_format((float)$row['mean'], 2, '.', ''). " €" . "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";                            
                            echo "</table>";
