        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="page-header clearfix">
                        <h2 class="pull-left">Trusted product per category</h2>
                    </div>
                    <?php
                    // Include config file
                    ...
                    $sql = "SELECT catType, d.per as pper from category,
                    (SELECT categoryID, 100*labeledNum/ COUNT(*) as per FROM
                    (SELECT categoryID,  sum(labeled) over (partition by categoryID) as labeledNum FROM 
                    (SELECT product.barcode, categoryID, product.labeled FROM product,
                    (SELECT barcode from contains) as a
                    WHERE product.barcode = a.barcode) as b) as c group by categoryID) as d
                        ...
                        if(mysqli_num_rows($result) > 0){
                            echo "<table class='table table-bordered table-striped'>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                        ...
                                        echo "<td>" . number_format((float)$row['pper'], 2, '.', ''). " %" . "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";                            
                            echo "</table>";
